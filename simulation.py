...
angles=np.arctan2(elecs[0,:],elecs[1,:])
...
Ekin_min=270
Ekin_max=330
H,xedges,yedges=np.histogram2d(angles,Ekin_final_list,[np.linspace(-np.pi,np.pi*15/16,16),np.linspace(Ekin_min,Ekin_max,30)])
...
plt.subplot(2,1,1)
plt.imshow(time_energy_map,extent=[min(times_list)*1e15,max(times_list)*1e15,min(Ekin_list),max(Ekin_list)],aspect='auto',origin='lower')
...
plt.grid(1,ls='--')
plt.scatter(angles,Ekin_final_list,c=birthtimes*1e15,cmap=cm.copper, marker='.', label=f'dummy')
