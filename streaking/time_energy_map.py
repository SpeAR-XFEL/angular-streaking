...
class Time_Energy_Map:
   ...
   def __init__(self,
    ...
    if image_path is not None:
        self.time_energy_map=gaussian_filter(np.flipud(-1*plt.imread(image_path)[:,:,0] + 1),sigma=3)
        
        self.Ekin_list=np.linspace(Ekin_range[0],Ekin_range[1],self.time_energy_map.shape[0])
        self.times_list=np.arange(time_range[0],time_range[1],self.time_energy_map.shape[1])
    ...
    self.time_list=np.linspace(time_min,time_max,np.rint((time_max-time_min)/stepsizes[0]).astype(int)+1)
    self.Ekin_list=np.linspace(Ekin_min,Ekin_max,np.rint((Ekin_max-Ekin_min)/stepsizes[1]).astype(int)+1)
    ...
    for i in range(N_pulses):
        ...
        corr=corr_list[i]
        I=I_list[i]
        time_energy_map=time_energy_map+I/(2*np.pi*sigma1*sigma2*np.sqrt(1-corr**2))*np.exp(-1/(2-2*corr**2)*((X-mu1)**2/sigma1**2+(Y-mu2)**2/sigma2**2-2*corr*(X-mu1)*(Y-mu2)/sigma1/sigma2))
